...
contract ConfigurableToken is StandardToken, Ownable
{
 ...
 /// @dev release Advisors tokens on advisors address
  function releaseTokens(address _beneficiary, uint256 _tokens) public onlyOwner returns (bool) 
  {
    ...
    emit Transfer(address(this), _beneficiary, tokens);
    return true;
  }
}

contract TokenB is ConfigurableToken
{
  string public constant name = "TokenB";
  string public constant symbol = "TokenB";
  uint32 public constant decimals = 18;

  function TokenB(address _newOwner) public 
  {
    require(_newOwner != address(0));
    owner = _newOwner;
    balances[_newOwner] = balances[_newOwner].add(1000 * 1 ether);
  }
}
